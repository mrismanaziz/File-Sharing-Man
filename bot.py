...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANEL:
            try:
                link = (await self.get_chat(FORCE_SUB_CHANEL)).invite_link
                if not link:
                    await self.export_chat_invite_link(FORCE_SUB_CHANEL)
                    link = (await self.get_chat(FORCE_SUB_CHANEL)).invite_link
                self.invitelink2 = link
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning(
                    "Bot tidak dapat Mengambil link invite dari FORCE_SUB_CHANEL!"
                )
                self.LOGGER(__name__).warning(
                    f"Pastikan @{self.username} adalah admin di Group Tersebut, Chat ID F-Subs chanel Saat Ini: {FORCE_SUB_CHANEL}"
                )
                self.LOGGER(__name__).info(
                    "Bot Berhenti. Gabung Group https://t.me/SharingUserbot untuk Bantuan"
                )
